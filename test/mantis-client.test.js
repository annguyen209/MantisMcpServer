...
function setEnv() {
  ...
  process.env.MANTIS_API_TOKEN = 'test-token';
}
...
test('usersMe calls /users/me', async () => {
  ...
  try {
    ...
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/users/me');
...
test('projectsMe calls /projects', async () => {
  ...
  try {
    await projectsMe();
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/projects');
  } finally {
    restore();
  }
});
...
test('healthCheck returns config context and user payload', async () => {
  ...
  try {
    ...
    assert.equal(result.data.authenticated_user.id, 77);
  } finally {
    restore();
  }
});
...
test('issuesSearch forwards query filters', async () => {
  ...
  try {
    await issuesSearch({ project_id: 50, page: 2, page_size: 25 });
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/issues?project_id=50&page=2&page_size=25');
  } finally {
    restore();
  }
});
...
test('issuesAssignedToMe resolves /users/me then /issues?handler_id=...', async () => {
  ...
  try {
    ...
    assert.equal(calledUrls[0], 'https://mantis.example.com/api/rest/index.php/users/me');
    assert.equal(
      calledUrls[1],
      'https://mantis.example.com/api/rest/index.php/issues?handler_id=9&page=3&page_size=10&severity_id=50'
    );
...
test('issuesUpdate patches /issues/{id}', async () => {
  ...
  try {
    ...
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/issues/999');
...
test('issuesTimeLogAdd calls TimeTracking/timesheet_crud_api with query params', async () => {
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      'https://mantis.example.com/plugin.php?page=TimeTracking%2Ftimesheet_crud_api&issue_id=8827&duration=01%3A30&note_text=Worked+on+validation+rules&category=Development&time_exp_date=2026-03-17'
    );
...
test('issuesTimeLogAdd supports CRUD action/read with record_id', async () => {
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      'https://mantis.example.com/plugin.php?page=TimeTracking%2Ftimesheet_crud_api&action=read&record_id=145'
    );
    assert.equal(calledInit.method, 'GET');
    assert.equal(result.status, 200);
  } finally {
...
test('timesheetReportQuery calls TimeTracking plugin endpoint with filters', async () => {
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      'https://mantis.example.com/plugin.php?page=TimeTracking%2Ftimesheet_report_api&format=json&report_page=2&report_page_size=50&time_filter_from=2026-03-01&time_filter_to=2026-03-17&time_filter_user_id=5&time_filter_category=Development&time_filter_overtime=No&project_id=50'
    );
    assert.equal(result.status, 200);
  } finally {
    restore();
...
test('issuesDelete calls DELETE /issues?id=...', async () => {
  ...
  try {
    ...
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/issues?id=991');
...
test('configGet calls /config with repeated option values', async () => {
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      'https://mantis.example.com/api/rest/index.php/config?option=timezone&option=window_title&project_id=50&user_id=1'
    );
  } finally {
    restore();
  }
...
test('langGet calls /lang?string=...', async () => {
  ...
  try {
    ...
    assert.equal(calledUrl, 'https://mantis.example.com/api/rest/index.php/lang?string=bug&string=project');
  } finally {
    restore();
  }
});
  ...
  try {
    await assert.rejects(
      ...
      (err) => {
        ...
        assert.equal(err.url, 'https://mantis.example.com/api/rest/index.php/users/me');
...
const liveMantisEnv = {
  MANTIS_BASE_URL: 'https://localhost:4430/api/rest',
  MANTIS_USE_INDEX_PHP: 'true',
  MANTIS_API_TOKEN: 'zCL5hxcj7adus7ezc8IGzN1-dYThOPIZ',
